...
public class AddressValidation : AbstractValidator<Address>
{
    public AddressValidation()
    {
        ...
        RuleFor(c => c.PostalCode)
            ...
            .Matches(@"^\d{5}-?\d{3}$")
            .WithMessage(
                "The field {PropertyName} must be a valid postal code (e.g., 12345-678 or 12345678)."
            );
        ...
        When(
            c => !string.IsNullOrEmpty(c.Complement),
            () =>
            {
                RuleFor(c => c.Complement)
                    .MaximumLength(100)
                    .WithMessage(
                        "The field {PropertyName} must not exceed {MaxLength} characters."
                    );
            }
        );
    }
