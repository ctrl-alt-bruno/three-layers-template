...
public class ProductValidation : AbstractValidator<Product>
{
    public ProductValidation()
    {
        ...
        RuleFor(c => c.SupplierId)
            .NotEmpty().WithMessage("The field '{PropertyName}' is required.")
            .NotEqual(Guid.Empty).WithMessage("The field '{PropertyName}' must be a valid GUID.");

        RuleFor(c => c.CreationDate)
            .LessThanOrEqualTo(DateTime.UtcNow).WithMessage("The field '{PropertyName}' cannot be in the future.");
    }
}