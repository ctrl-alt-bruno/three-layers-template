...
public class SupplierValidation : AbstractValidator<Supplier>
{
    public SupplierValidation()
    {
        ...
        RuleFor(c => c.Document)
            ...
            .WithMessage("The document is not valid.");

        RuleFor(c => c.SupplierType)
            .IsInEnum()
            .WithMessage("The field {PropertyName} must be a valid supplier type.");

        When(
            x => x.SupplierType == SupplierTypes.Regular,
            () =>
            {
                RuleFor(x => x.Document)
                    .Length(11)
                    .WithMessage("CPF must have exactly {ExpectedLength} characters.")
                    .Must(DocumentsValidator.ValidateCpf)
                    .WithMessage("CPF is not valid.");
            }
        );

        When(
            x => x.SupplierType == SupplierTypes.Legal,
            () =>
            {
                RuleFor(x => x.Document)
                    .Length(14)
                    .WithMessage("CNPJ must have exactly {ExpectedLength} characters.")
                    .Must(DocumentsValidator.ValidateCnpj)
                    .WithMessage("CNPJ is not valid.");
            }
        );
    }
